...
def make_param_group_fn(
    ...
    The remaining parameters use ``base_lr``. If ``filter_bias_and_bn`` is
    ``True`` (default), biases and 1D tensors (typically norms) are placed in a
    ...
    def param_group_fn(model: torch.nn.Module) -> List[dict]:
        ...
        def _add_group(named: Iterator[Tuple[str, torch.nn.Parameter]], lr: float) -> None:
        ...
        for prefix, lr in param_groups.items():
            ...
            _add_group(params, lr or base_lr)
