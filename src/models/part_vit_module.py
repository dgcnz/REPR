...
class PARTViTModule(LightningModule):
    ...
    def model_step(
        ...
        x, patch_positions = sample_and_stitch(x_original, self.hparams.patch_size, self.hparams.sample_mode)
        pred_T, patch_pair_indices = self.forward(x)
        gt_T = compute_gt_transform(patch_pair_indices, patch_positions)
        loss = self.criterion(pred_T, gt_T)
        return loss, pred_T, gt_T
    ...
    def on_train_epoch_end(self) -> None:
        "Lightning hook that is calle when a training epoch ends."
        pass
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        self.log("test/acc", self.test_mse, on_step=False, on_epoch=True, prog_bar=True)
