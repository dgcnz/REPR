...
def get_all_pairs_subset(ids: Int[Tensor, "B N"]) -> Int[Tensor, "B N**2 2"]:
    B, N = ids.shape
    p1 = ids.unsqueeze(2).expand(B, N, N)
    p2 = ids.unsqueeze(1).expand(B, N, N)
    patch_pair_indices = torch.stack([p1, p2], dim=-1)  # Shape: [B, N, N, 2]
    patch_pair_indices = patch_pair_indices.reshape(B, N * N, 2)
    return patch_pair_indices
...
def get_all_pairs_by_ongrid_regions(
    ...
    if add_inter_region_pairs:
        first_patch_in_each_region = torch.arange(
            0, n_patches, n_patches_per_region, device=device
        )
        # generate all pairs between all first patches in each region
        inter_region_pairs = torch.stack(
            torch.meshgrid(first_patch_in_each_region, first_patch_in_each_region),
            dim=-1,
        ).reshape(-1, 2)
