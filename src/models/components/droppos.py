...
class DropPositionMaskedAutoEncoderViT(nn.Module):
    def __init__(
        ...
        self.decoder_pred = nn.Linear(
            decoder_embed_dim, num_patches, bias=True
        )  # decoder to patch
        # --------------------------------------------------------------------------
        ...
        if multi_task:
            ...
            self.aux_decoder_pos_embed = nn.Parameter(
            ...
            self.aux_decoder_pred = nn.Linear(
    ...
    def initialize_weights(self):
        ...
        if self.multi_task:
            decoder_pos_embed = get_2d_sincos_pos_embed(
                self.aux_decoder_pos_embed.shape[-1],
                int(self.patch_embed.num_patches**0.5),
                cls_token=True,
            )
            self.aux_decoder_pos_embed.data.copy_(
    ...
    def forward_encoder(self, x, mask_ratio, pos_mask_ratio):
        ...
        outs["mask"], outs["ids_keep"], outs["ids_restore"] = (
            mask,
            ids_keep,
            ids_restore,
        )
        ...
        outs["mask_pos"], outs["ids_keep_pos"], outs["ids_restore_pos"] = (
            mask_pos,
            ids_keep_pos,
            ids_restore_pos,
        )
        ...
        if self.mask_token_type == "param":
            ...
            mask_pos_tokens = torch.zeros((N, mask_pos_length, self.embed_dim)).to(
                x.device
            )
        elif self.mask_token_type == "wrong_pos":
            removed_pos_embed = torch.gather(
                pos_embed_vis, dim=1, index=ids_remove_pos.unsqueeze(-1).repeat(1, 1, D)
            )
            # convert to numpy, since numpy shuffles the first dimension, we have to transpose first
            removed_pos_embed = (
                removed_pos_embed.detach().cpu().permute(1, 0, 2).numpy()
            )  # [N, L, D] -> [L, N, D]
            np.random.shuffle(removed_pos_embed)
            # restore to torch
            removed_pos_embed = torch.from_numpy(removed_pos_embed).permute(
                1, 0, 2
            )  # [L, N, D] -> [N, L, D]
        ...
        if self.shuffle:
            ...
            ids_keep_shuffle, _, ids_restore_shuffle, _ = self.random_masking(x, 0.0)

            # gather
            x = torch.gather(
                x, dim=1, index=ids_keep_shuffle.unsqueeze(-1).repeat(1, 1, D)
            )
    ...
    def forward_decoder(self, outs):
        ...
        if self.shuffle:
            ...
            x = torch.gather(
                x,
                dim=1,
                index=ids_restore_shuffle.unsqueeze(-1).repeat(1, 1, x.shape[-1]),
            )
        # --------------------------------------------------------------------------
        ...
        if self.multi_task:
            ...
            mask_tokens = self.mask_token.repeat(
                x_mae.shape[0], outs["ids_restore"].shape[1] + 1 - x_mae.shape[1], 1
            )
            x_ = torch.cat([x_mae[:, 1:, :], mask_tokens], dim=1)  # no cls token
            x_ = torch.gather(
                x_,
                dim=1,
                index=outs["ids_restore"].unsqueeze(-1).repeat(1, 1, x_mae.shape[2]),
            )  # unshuffle
    ...
    def forward_drop_pos_loss(self, pred, mask, ids_keep, mask_pos, smooth, attn):
        ...
        labels_smooth = torch.gather(
            smooth.repeat(N, 1, 1), dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, L)
        )
        log_prob = F.log_softmax(pred, dim=-1)  # [N, x, L]
        if self.conf_ignore:
            # ignore if confidence > smooth
            conf_thresh = (
                torch.diag(smooth).cuda().detach().unsqueeze(0).repeat(N, 1)
            )  # [N, L]
            conf_thresh = torch.gather(conf_thresh, dim=1, index=ids_keep)  # [N, x]
            with torch.no_grad():
                prob = F.softmax(pred, dim=-1)  # [N, x, L]
                conf = torch.gather(
                    prob, dim=2, index=ids_keep.unsqueeze(1).repeat(1, num_vis, 1)
                )  # [N, x, x]
        ...
        # TODO: add this to metrics/droppos.py
        with torch.no_grad():
    ...
    def forward(self, imgs, sigma=0):
        num_patches = (self.img_size // self.patch_size) ** 2
        smooth = _get_label_smoothing_map(int(num_patches), sigma)

        outs = self.forward_encoder(imgs, self.mask_ratio, self.pos_mask_ratio)
        pred = self.forward_decoder(outs)
        if self.multi_task:
            acc1, loss_drop_pos = self.forward_drop_pos_loss(
                pred[0],
                outs["mask"],
                outs["ids_keep"],
                outs["mask_pos"],
        ...
        return self.forward_drop_pos_loss(
            pred,
            outs["mask"],
            outs["ids_keep"],
            outs["mask_pos"],
...
DropPos_mae_vit_base_patch16 = (
    DropPos_mae_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
DropPos_mae_vit_large_patch16 = (
    DropPos_mae_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
DropPos_mae_vit_huge_patch14 = (
    DropPos_mae_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
)
